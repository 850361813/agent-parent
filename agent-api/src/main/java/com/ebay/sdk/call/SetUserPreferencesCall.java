 ...
 * <br> <B>Input property:</B> <code>BidderNoticePreferences</code> - Container consisting of the seller's preference for receiving contact information for unsuccessful bidders. This preference is only applicable for auction listings.
 * <br> <B>Input property:</B> <code>CombinedPaymentPreferences</code> - Container consisting of the seller's preference for allowing Combined Invoice orders for the same seller and buyer. <br><br> <span class="tablenote"><strong>Note:</strong> Calculated and flat-rate shipping preferences are no longer set using this call. Instead, use the <b>SetShippingDiscountProfiles</b> call to set the shipping discounts for Combined Invoice orders. </span> <br> <span class="tablenote"><strong>Note:</strong> A seller's combined payment preferences can take up to 7 days to have any affect on eBay.
 * </span>
 * <br> <B>Input property:</B> <code>CrossPromotionPreferences</code> - This container is deprecated.
 * <br> <B>Input property:</B> <code>SellerPaymentPreferences</code> - Container consisting of the seller's payment preferences. One or more preferences may be set or modified under this field. Payment preferences specified in a <b>SetUserPreferences</b> call override the settings in My eBay payment preferences.
 * <br> <B>Input property:</B> <code>SellerFavoriteItemPreferences</code> - Container consisting of the seller's preferences for displaying items on a buyer's Favorite Sellers' Items page or Favorite Sellers' Items digest. One or more preferences may be set or modified under this field.
 * <br> <B>Input property:</B> <code>EndOfAuctionEmailPreferences</code> - Container consisting of the seller's preferences for the end-of-auction email sent to the winning bidder. These preferences allow the seller to customize the Email that is sent to buyer at the end of the auction. One or more preferences may be set or modified under this field. These preferences are only applicable for auction listings.
 * <br> <B>Input property:</B> <code>EmailShipmentTrackingNumberPreference</code> - Flag that controls whether the shipment's tracking number is sent by Email to the buyer.
 * <br> <B>Input property:</B> <code>RequiredShipPhoneNumberPreference</code> - Flag that controls whether the buyer is required to provide a shipping phone number upon checkout. Some shipping carriers require the receiver's phone number.
 * <br> <B>Input property:</B> <code>UnpaidItemAssistancePreferences</code> - Container consisting of a seller's Unpaid Item Assistant preferences. The Unpaid Item Assistant automatically opens an Unpaid Item dispute on the behalf of the seller. One or more preferences may be set or modified under this field.
 ...
 * <br> <B>Input property:</B> <code>SellerThirdPartyCheckoutDisabled</code> - A flag used to disable the use of a third-party application to handle the checkout flow for a seller. If set to true, Third-Party Checkout is disabled and any checkout flow initiated on the seller's application is redirected to the eBay checkout flow.
 ...
 * <br> <B>Input property:</B> <code>OutOfStockControlPreference</code> - When this flag is set to 'true', it enable the Out-of-Stock feature. A seller would use this feature to keep Fixed-Price GTC (Good 'Til Canceled) listings alive even when the "quantity available" value goes to 0 (zero). This is useful when waiting for additional stock and eliminates the need to end the listing and then recreating it when stock arrives. <br/><br/>
