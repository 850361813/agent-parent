 ...
 * <br> <B>Input property:</B> <code>ScheduledList</code> - Returns the list of items the user has scheduled to sell but whose listings have not yet opened. <br><br> Set <b>Include</b> to <code>true</code> to return the default response set.
 * <br> <B>Input property:</B> <code>ActiveList</code> - Returns the list of items the user is actively selling (the currently active listings). <br><br> Set <b>Include</b> to <code>true</code> to return the default response set.
 * <br> <B>Input property:</B> <code>SoldList</code> - Returns the list of items the user has sold.
 * <br><br>
 * Set <b>Include</b> to <code>true</code> to return the default response set.
 ...
 * <br> <B>Input property:</B> <code>HideVariations</code> - If true, the <b>Variations</b> node is omitted for all multi-variation listings in the response. If false, the <b>Variations</b> node is returned for all multi-variation listings in the response.
 * <br> <br>
 * Please note that if the seller includes a large number of variations in many listings, retrieving variations (setting this flag to <code>false</code>) may degrade the call's performance. Therefore, when this is false, you may need to reduce the total number of items you're requesting at once (by using other input fields, such as <b>Pagination</b>).
